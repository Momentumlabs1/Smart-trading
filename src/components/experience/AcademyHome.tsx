...
import SaifFunnel, { summarize } from '../funnel/SaifFunnel';
...
export default function AcademyHome(){
  ...
  const play=(start='begruessung')=>{lastFocus.current=document.activeElement as HTMLElement;setVideo(start);};
  const request=(topic='Erstgespräch',summary='')=>{lastFocus.current=document.activeElement as HTMLElement;setContext(summary);setContact(topic);};
  ...
  </main><ExperienceFooter onContact={()=>request()}/>{video!==null&&<SaifFunnel open start={video} onClose={()=>setVideo(null)} returnFocus={lastFocus.current} onContact={(topic,answers)=>{setVideo(null);window.setTimeout(()=>request(topic,summarize(answers)),120);}}/>}<ContactDialog key={contact||'closed'} topic={contact} context={context} close={()=>setContact(null)} returnFocus={lastFocus.current}/></div>;
}
