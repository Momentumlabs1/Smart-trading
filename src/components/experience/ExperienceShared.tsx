...
export function ExperienceFooter({onContact}: {onContact:()=>void}) {
  return <footer className="sx-footer sx-wrap"><div className="sx-footer-top"><SaifMark/><p>DEIN WEG. DEIN SYSTEM.</p><button onClick={onContact}>Kontakt aufnehmen <ArrowUpRight size={17}/></button></div><div className="sx-footer-links"><a href="/#lernweg">Academy-Lernweg</a><Link to="/signale">Telegram-Gruppe & Basic Academy</Link><a href="https://academy.smart-trading.at" target="_blank" rel="noopener noreferrer">Mitglieder-Login</a></div><div className="sx-footer-legal"><Link to="/impressum">Impressum</Link><Link to="/datenschutz">Datenschutz</Link><Link to="/risikohinweis">Risikohinweis</Link></div><div className="sx-footer-bottom"><p>Trading birgt Verlustrisiken. Saifs Trades und Lerninhalte sind keine Anlageberatung und keine Gewinngarantie. <Link to="/risikohinweis">Risikohinweis</Link></p><span>© {new Date().getFullYear()} SAIF SMART TRADING</span></div></footer>;
}
